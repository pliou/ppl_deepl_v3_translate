...
final class TranslatedDownloadStorage
{
    ...
    public function createToken(string $storageFileName, string $displayName): string
    {
        ...
        $signature = GeneralUtility::makeInstance(\TYPO3\CMS\Core\Crypto\HashService::class)->hmac($encoded, self::SECRET_SCOPE);
    ...
    public function resolveToken(string $token): ?array
    {
        ...
        if (!hash_equals(GeneralUtility::makeInstance(\TYPO3\CMS\Core\Crypto\HashService::class)->hmac($encoded, self::SECRET_SCOPE), $signature)) {
            return null;
        }
